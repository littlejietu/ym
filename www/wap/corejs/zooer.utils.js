...
function jump_to_url(url) {
    ...
    if (url.indexOf('shop_id') < 0 && !deviceInfo) {
        url += '&shop_id=' + get_string_fromlocal('shop_id');
...
function add_goods_to_cart(goods_id, sku_id, num1, fun) {
    ...
    sendPostData({goods_id: goods_id, sku_id: sku_id, num: num1}, ApiUrl + 'm/cart/add', function (result) {
        if (result.code == 1) {
            ...
            showTips('加入购物车成功！');
            //sendPostData({shop_id:get_string_fromlocal('shop_id')},ApiUrl+'m/cart/',function(result){
            //    if(result.code == 1){
...
function sendPostData(obj, url, fun, errfun) {
    ...
    if (temp == 'discount_goods') {
        //alert(url);
        if (location.href.indexOf('shop_id') < 0) {
            obj.shop_id = get_string_fromlocal('shop_id')
        } else {
            obj.shop_id = getUrlParam("shop_id");
        }
        //return ;
...
function goHome(){
    location.href = WapSiteUrl + '/home/index.html';
}
