...
class fundOrder_service
{
	...
	private function initFundOrderBaseData($arrFundOrder){
		if(empty($arrFundOrder['fund_order_sn']))
			$arrFundOrder['fund_order_sn'] = getOrderSn();
	...
	private function checkOrder($arrFundOrder){
		...
		if($arrFundOrder['balance_amt']>0 && $aUserAccount['acct_balance']<$arrFundOrder['balance_amt']){
			$arrReturn['code'] = 'Failure';
	...
	public function doConsume($orderId, $orderSn, $title, $shop_id, $buyerUserId, $buyerUserName, $amount, $sellerUserId, $sellerUserName, $netpayMethod, $extParam, $ip, $platformId) {
		$arrReturn = array('code'=>'Empty','errInfo'=>'','fund_order_id'=>0);
		...
		if (empty($aFundOrder)) {
			...
			$oCreate = array('order_id'=>$orderId, 'order_sn'=>$orderSn,'type_id'=>C('OrderType.Consume'),
				'title'=>$title, 'shop_id'=>$shop_id, 'total_amt'=>$amount,'bonus_amt'=>0, 'netpay_amt'=>$netpay_amt,
			...
			if($aFundOrder['netpay_method']!=$netpayMethod){
				$data = array('netpay_method'=>$netpayMethod);
	...
	public function jump($payMethodName, $param) {
		...
		$aFundOrder = $this->Fundorder_model->get_by_id($payNoticeResult['fund_order_id']);
