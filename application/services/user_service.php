...
class User_service
{
    public function __construct()
	{
        ...
        $this->ci->load->model('User_detail_model');
		$this->ci->load->library('encryption');
        $this->ci->load->model('Deliver_user_model'); //派送员信息主表
        $this->ci->load->model('Deliver_user_pwd_model'); //派送员密码表
...
//             return $arrRes;
    ...
    /**派送员注册*/
    public function reg_deliveruser($arrUserData){

        $where['user_name'] = "'".$arrUserData['mobile']."'";
        $arrUserInfo = $this->ci->Deliver_user_pwd_model->get_by_where($where);
        if(!empty($arrUserInfo)) {
            $UserInfo = $this->ci->Deliver_user_model->get_by_where('user_id = '.$arrUserInfo['id']);
            $arrUserInfo['logo'] = $UserInfo['logo'];
            return $arrUserInfo;
        }

        if (!preg_match("/^1[34578]\d{9}$/",$arrUserData['mobile']))
        {
            output_error(-1,'USER_PHONE_FORMAT_ERROR');exit;
        }

            $user_pwd_data = array(
                'user_name' => $arrUserData['mobile'],
                'status' => 0,
                'platform_id' => 1,
            );
            $user_pwd_data['token'] = md5(time().mt_rand(0,1000));
            if ($insert_id = $this->ci->Deliver_user_pwd_model->insert_string($user_pwd_data))
            {
                $data = array(
                    'user_id'       => $insert_id,
                    'user_name'     => $arrUserData['mobile'],
                    ...
                    'mobile_verify' => 1,
                    'reg_time'      => time(),
                    'reg_ip'        => $arrUserData['ip'],
                    'update_time'   => time(),
                );

                $this->ci->Deliver_user_model->insert($data);
                $info = $this->ci->Deliver_user_pwd_model->get_by_where($where);
