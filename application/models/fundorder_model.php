	...
	private function refundSettled($arrFundOrder){
		...
		if ($arrFundOrder['type_id'] == C('OrderType.Consume')) {
			...
			if($arrFundOrder['status']==C('FundOrderStatus.WaitingSettle')){
				...
				//2.1.2买家退款
				$remark = $arrFundOrder['title'].'退款';
				if ($refundAmount > 0 && !$this->accountBalanceChange($arrFundOrder['buyer_userid'], $arrFundOrder['order_id'], $arrFundOrder['fund_order_id'], C('BalanceLogType.RefundTradeIn'), $refundAmount, $remark, $arrFundOrder['platform_id'])) {
					$this->db->trans_rollback();
					return false;
				}
			...
			//2.2 已结算，退货退钱
				...
				if ($refundAmount > 0 && !$this->accountBalanceChange($arrFundOrder['buyer_userid'], $arrFundOrder['order_id'], $arrFundOrder['fund_order_id'], C('BalanceLogType.RefundTradeOut'), $refundAmount, $remark, $arrFundOrder['platform_id'])) {
					$this->db->trans_rollback();
					return false;
				}
				//2.2.3买家退款
