...
class Goods_tpl_add extends MY_Admin_Controller {
	...
	public function add_step1()
    {
        ...
        $tpl_id = $this->input->post_get('tpl_id');
        $goods_class = $this->Category_model->getShopList();

        $result = array(
            'output' => array('goods_class' => $goods_class,
                'tpl_id'=>$tpl_id
                )
        );


        $this->load->view('admin/goods_tpl/goods_add_step1', $result);
    }
    ...
    public function add_step2()
    {
        ...
        $this->load->model('Goods_tpl_sku_model');
        ...
        if($tpl_id)
        {
            $aGoods = $this->Goods_tpl_model->get_by_where(array('tpl_id'=>$tpl_id));
...
//            $aGoods = array_merge($aGoods_detail,$aGoods_num,$aGoods);
    ...
    //            unset($aGoods_num);
            ...
            if(!empty($aGoods['transport_id'])){
                $aTransport = $this->Transport_model->get_by_id($aGoods['transport_id']);
                if(!empty($aTransport))
                    $aGoods['transport_title'] = $aTransport['title'];
            }
            ...
            //sp_value-->库存配置
            $sp_value = array();
            $aSku = $this->Goods_tpl_sku_model->get_list(array('goods_id'=>$tpl_id),'*');
            if(!empty($aSku))
            {
                foreach ($aSku as $v) {
                    ...
                    $sp_value ['i_' . $code . '|commprecent'] = $v['comm_precent'];
                }
            }
            ...
            //选中的规格
            $whereattrvar = array('goods_id'=>$tpl_id);
            $aSpec = $this->Goods_tpl_spec_attr_val_model->get_list($whereattrvar,'val_id,spec_val,pic','sort asc,id desc');
            if(!empty($aSpec))
            {
                $spec_checked = array();
        ...
        Tpl::showpage('goods_tpl/goods_add_step2','goods_tpl_layout','admin');
    }
    ...
    public function save() {
        ...
        if ($this->input->is_post())
        {
            ...
            if ($this->form_validation->run() === TRUE)
            {
                ...
                $this->load->model('Spu_model');
                $this->load->model('Goods_tpl_model'); //产品主表
               // $this->load->model('Goods_detail_model');
                //$this->load->model('Goods_num_model');  //统计
                $this->load->model('Goods_tpl_spu_attr_val_model');
                //$this->load->model('Goods_spec_attr_name');
                $this->load->model('Goods_tpl_spec_attr_val_model');
                $this->load->model('Goods_tpl_sku_model');  //库存
                ...
                //goods
                $data = array(
                    'title'         => $this->input->post( 'title' ),   //商品标题
    				'point'         => $this->input->post( 'point' ),   //卖点
    				'price'         => round( floatval($this->input->post( 'price' ) ), 2), //一口价
                    'market_price'  => round( floatval($this->input->post( 'market_price' )), 2),    //市场价
                    'cost_price'    => round( floatval($this->input->post( 'cost_price' )), 2),    //成本价
                    'comm_price'    => round( floatval( $this->input->post( 'comm_price' )), 2),  //利润价
                    'comm_precent'  => round( floatval( $this->input->post( 'comm_precent' )), 2),  //利润比率
    				'discount'      => $this->input->post( 'discount' ), //折扣率
    				'brand_id'      => $this->input->post( 'b_id' ), //品牌id
    				'shop_id'       => $shop_id,  //所属店铺id
    				'category_id'   => $cid,  //最底类目id
    				'category_id_1' => $arrCategory['id_1'],    //一级分类
    				'category_id_2' => $arrCategory['id_2'],    //二级分类
    				'category_id_3' => $arrCategory['id_3'],    //三级分类
//    				'trade_coding'  => $this->input->post( 'trade_coding' ),
    				'barcode'       => $this->input->post( 'barcode' ),   //商品条形码
    				'pic_path'      => $this->input->post( 'image_path' ),   //主图路径
    				//'video' => $this->input->post( 'video' ), //视频
                    'spu_id'        => $spu_id,
//    				'is_presell' => 0,
//    				'is_own' => 1,
                    'stock_num'     => $this->input->post( 'stock_num' ),
                    'transport_id'  => $this->input->post( 'transport_id' ),
    				'status'        => $g_state,  //状态 1:上架 2:下架
                    'content'       => $this->input->post( 'content' ),
                    'm_content'     => $m_content,
                    'service_sort'  => $this->input->post( 'service_sort' ),
    				'addtime'       => time(),    //添加时间
                ...
                if($tpl_id){
                    $this->Goods_tpl_model->update_by_where(array('tpl_id'=>$tpl_id), $data);
                }else{
                    $tpl_id = $this->Goods_tpl_model->insert_string($data);
                }
                //-goods

                //goods_detail  模板无产品详细
               ...
               /* $data_num = array(
                ...
                if(is_array($arrSpecName)){
                    ...
                    foreach ($arrSpecName as $name_id=>$spec_name) {
                        ...
                        $this->Goods_spec_attr_name_model->insert($data_spec_name);
                    }
                }*/
                ...
                if(is_array($arrSpecVal)){
                    foreach ($arrSpecVal as $name_id=>$aSpecVal) {
                        ...
                        foreach ($aSpecVal as $key => $value) {
                            ...
                            $this->Goods_tpl_spec_attr_val_model->insert_update($data_spec_val);
                        ...
                        if(!empty($arrValId))
                            $this->Goods_tpl_spec_attr_val_model->delete_by_where("goods_id=$tpl_id and name_id=$name_id and val_id not in(".implode(',', $arrValId).")");
                    }
                }
                else if(empty($arrSpecVal)){   //没有选中规格
                    $this->Goods_tpl_spec_attr_val_model->delete_by_where("goods_id=$tpl_id");
                }

                //-goods_spec

                //goods_sku  模板存规格组合，无库存
                $strSku_code = '';
                if(!empty($arrSpecVal) && is_array($arrSku) ) {
                    $word_model = M('Word');
                    ...
                    foreach ($arrSku as $aSku) {
                        ...
                        $sku_code = $tpl_id.':'.implode('_', array_keys($aSku['sp_value']));

                        $comm_price = ($aSku['price'] - $aSku['costprice'])*$aSku['commprecent']/100;
                        $comm_price = $comm_price<0?0:$comm_price;
                        $data_sku = array(
                            'goods_id'=>$tpl_id,
                            'sku_title'=>$sku_title,
                            ...
                            'cost_price'=>$aSku['costprice'],
                            'comm_precent'=>$aSku['commprecent'],
                            'comm_price'=>$comm_price,
                        );
                        $this->Goods_tpl_sku_model->insert($data_sku);
                    ...
                    if(!empty($strSku_code) && $strSku_code != ','){
                        ...
                        $this->Goods_tpl_sku_model->delete_by_where("goods_id=$tpl_id and sku_code not in($strSku_code)");
                    }
                }
                if(empty($strSku_code)){
                    //没有选中规格
                    $this->Goods_tpl_sku_model->delete_by_where("goods_id=$tpl_id");
                }
                //-goods_sku
                //goods_spu
                $attrValList = $this->Spu_model->getAttrValBySpuCode($spu_code);
                foreach ($attrValList as $key => $a) {
                    ...
                    $this->Goods_tpl_spu_attr_val_model->insert_update($data_spu);
                }
                //-goods_spu

                //sync
                $this->sync_service->tplToGoodsAll($tpl_id);

                //redirect($gotoUrl);
               
                $gotoUrl = urlShop('admin/goods_tpl_add','add_step3',array('tpl_id' => $tpl_id));
            ...
            else
            {
                ...
                $gotoUrl = urlShop('admin/goods_tpl_add','add_step1',array('tpl_id' => $tpl_id));
    ...
    public function add_step3(){
        ...
        $goods_pic = '';
        $aGoods = $this->Goods_tpl_model->get_by_where(array('tpl_id'=>$tpl_id),'pic_path');
        if(!empty($aGoods))
            $goods_pic = $aGoods['pic_path'];
        Tpl::output('goods_pic',$goods_pic);

        //$cid = $aGoods['category_id'];
        $pic_list = $this->Goods_tpl_pic_model->get_list(array('goods_id'=>$tpl_id),'*','sort asc,id asc');

        Tpl::output('goods_id',$tpl_id);
        Tpl::output('pic_list',$pic_list);

        Tpl::showpage('/goods_tpl/goods_add_step3','goods_tpl_layout','admin');
...
public function save_image(){
        ...
        foreach ($arrImg as $key => $value) {
            ...
            // {
                ...
                $this->Goods_tpl_pic_model->insert($data);
            ...
            if($value['default']==1)
            {
                $this->Goods_tpl_model->update_by_where(array('tpl_id'=>$goods_id), array('pic_path'=>$value['path']));
                $this->Goods_model->update_by_where(array('tpl_id'=>$goods_id), array('pic_path'=>$value['path']));
            }
        }

        go_redirect(urlShop('admin/goods_tpl_add', 'add_step4', array('tpl_id' => $goods_id)));
    }
    ...
    public function add_step4(){
        ...
        $this->lang->load(array('user_goods','common'));
        Tpl::output('goods_id',$tpl_id);
        Tpl::showpage('/goods_tpl/goods_add_step4','goods_tpl_layout','admin');
    }
