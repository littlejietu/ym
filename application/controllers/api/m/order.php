...
//	    echo json_encode($result);
    ...
    public function paying(){
        ...
        $arrReturn = $this->order_service->gotoPay($orderIds, $paymethod, $ip);
    ...
    public function addlogistic(){
        ...
        $id = $this->input->post('id');  //退款主键ID
        $logistic_name = $this->input->post('logistic_name');
        $logistic_sn = $this->input->post('logistic_sn');
        $phone = $this->input->post('phone');

        if(empty($id)){

            output_error(-1,'id不能为空'); //ID_NULL
            exit;
        }

        if(empty($logistic_sn)){

            output_error(-2,'物流单号不能为空');    //LOGISTIC_ID_NULL
            exit;
        }

        if(empty($logistic_name)){

            output_error(-3,'物流名称不能为空');  //LOGISTIC_NAME_NULL
            exit;
        }

        if(empty($phone)){

            output_error(-4,'电话不能为空');  //PHONE_NULL
            exit;
        }

        $aRefund = $this->Order_refunds_model->get_by_id($id);
        if(empty($aRefund)){
            output_error(-4,'该退货申请不存在');
            exit;
        }

        if($aRefund['status']!=3){
            output_error(-5,'等待商家同意');
            exit;
        }

        $inData = array(
            'logistic_name'     => $logistic_name,
            'logistic_sn'       => $logistic_sn,
            'phone'             => $phone,
            'status'            => 4,
            'lasttime'          => time(),
        );

        $this->Order_refunds_model->update_by_id($id,$inData);
        output_data();
    }
