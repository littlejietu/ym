...
<form method="post" id="goods_image" action="<?php echo urlShop('seller/goods_add', 'save_image');?>">
  ...
  <div class="ncsc-form-goods-pic">
    <div class="container">
      <div class="ncsc-goodspic-list">
        <ul nctype="ul">
          ...
          <li class="ncsc-goodspic-upload">
            <input type="hidden" name="img[<?php echo $i;?>][id]" value="<?php if(!empty($a)) echo $a['id'];?>"><!-- nctype="file_<?php //echo  $i;?>"-->
            <div class="upload-thumb"><img src="<?php if(!empty($a)) echo cthumb($a['pic']);?>" nctype="file_<?php echo  $i;?>">
              <input type="hidden" name="img[<?php echo $i;?>][path]" value="<?php if(!empty($a)) echo $a['pic'];?>" nctype="file_<?php echo  $i;?>">
            </div>
        ...
        <div class="ncsc-select-album">
          <a class="ncsc-btn" href="<?php echo urlShop('seller/goods_album', 'pic_list', array('item'=>'goods_image'));?>" nctype="select"><i class="icon-picture"></i>从图片空间选择</a>
     ...
     <div class="bottom tc hr32"><label class="submit-border"><input type="submit" class="submit" value="提交" /></label></div>
    </div>
...
<?php echo _get_html_cssjs('seller_js','jquery.ajaxContent.pack.js,shop_goods_add.step3.js','js');?>
<script>
