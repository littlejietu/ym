...
<?php echo _get_html_cssjs('seller_js','jquery.poshytip.min.js,jquery.mousewheel.js,jquery.charCount.js,shop_goods_add.step2.js','js');?>
...
<?php echo _get_html_cssjs('seller_js','ToolTip.js','js');?>
    ...
    <?php else:?>
      <div class="tabmenu">
        <ul class="tab pngFix">
          <li class="active"><a href="<?php echo urlShop('seller/goods_add','add_step2',array('id'=>$output['goods']['id']));?>">编辑商品</a></li>
          <!-- <li class="normal"><a href="<?php //echo urlShop('seller/goods_add','add_step3',array('id'=>$output['goods']['id']));?>">编辑图片</a></li> -->
...
<?php //echo validation_errors('<div class="valid_error">', '</div>');?>
      <div class="item-publish">
        <form method="post" id="goods_form" action="<?php echo urlShop('seller/goods_add', 'save');?>">
          ...
          <div class="ncsc-form-goods">
            ...
            <dl style="display:none" nc_type="spec_group_dl_<?php echo $i;?>" nctype="spec_group_dl" class="spec-bg" <?php if($val['type'] == '1'){?>spec_img="t"<?php }?>>
              <dt>
              ...
              <dd class="spec-dd">
                <table border="0" cellpadding="0" cellspacing="0" class="spec_table">
                  ...
                  <th nctype="spec_name_<?php echo $k;?>"><?php if (isset($output['goods']['spec_name'][$k])) { echo $output['goods']['spec_name'][$k];} else {echo $val['name'];}?></th>
                    ...
                    <th class="w100"><span class="red">*</span>成本价<div class="batch"><i class="icon-edit" title="批量操作"></i>
                        <div class="batch-input" style="display:none;">
                          <h6>批量设置库存：</h6>
                          <a href="javascript:void(0)" class="close">X</a>
                          <input name="" type="text" class="text price" />
      ...
      <dl>
              ...
              <dd id="gcategory"> <?php echo $output['goods_class']['line_cate_name'];?> <!-- <a class="ncsc-btn" href="<?php //echo SELLER_SITE_URL?>/goods_add/add_step1"><?php //echo lang('nc_edit');?></a> -->
              </dd>
            </dl>
            <dl>
              <dt><i class="required">*</i><?php echo lang('store_goods_index_goods_name').lang('nc_colon');?></dt>
              <dd>
                <input name="title" type="text" class="text w400" <?php if(is_tpl_follow('title')) echo "readonly"?> value="<?php echo empty($output['goods']['title'])?'':$output['goods']['title']; ?>" />
                <i class="hint"><?php if(is_tpl_follow('title')) echo lang('goods_attr_admin_control');?></i>
            ...
            <dl>
              ...
              <dd>
                <textarea name="point" class="textarea h60 w400" <?php if(is_tpl_follow('point')) echo "readonly"?> ><?php echo empty($output['goods']['point'])?'':$output['goods']['point']; ?></textarea>
                <i class="hint"><?php if(is_tpl_follow('title')) echo lang('goods_attr_admin_control');?></i><span></span>
            ...
            <dl style="display: none">
              <dt><i class="required">*</i>利润价<?php echo lang('nc_colon');?></dt>
              <dd>
                <input name="comm_price" <?php if(is_tpl_follow('comm_price')) echo "readonly"?> value="<?php echo empty($output['goods']['comm_price'])?'':$output['goods']['comm_price']; ?>" type="text" class="text w60" /><em class="add-on"><i class="icon-renminbi"></i></em> <span></span>
                <p class="hint"><?php if(is_tpl_follow('comm_price')) echo lang('goods_attr_admin_control');?></p>
              </dd>
            </dl>
            <dl style="display: none">
                ...
                <div class="ncsc-goods-default-pic">
                  <div class="goodspic-uplaod">
                    <div class="upload-thumb">
                        <img nctype="goods_image" src="<?php if(!empty($output['goods'])) echo thumb($output['goods'], 240);?>"/>
                    </div>
                    ...
                    <?php if(is_tpl_follow('pic_path')):
                      echo lang('goods_attr_admin_control');
                      else:?>
                    ...
                    <div class="handle">
                      ...
                      <a class="ncsc-btn mt5" nctype="show_image" href="<?php echo urlShop('seller/goods_album', 'pic_list', array('item'=>'goods'));?>"><i class="icon-picture"></i>从图片空间选择</a> <a href="javascript:void(0);" nctype="del_goods_demo" class="ncsc-btn mt5" style="display: none;"><i class="icon-circle-arrow-up"></i>关闭相册</a>
                    </div>
                    <?php endif?>
                  </div>
                </div>
            ...
            <!-- <dl style="overflow: visible;">
              <dt><?//php echo lang('store_goods_index_goods_brand').lang('nc_colon');?>1234</dt>
              <dd>
                <div class="ncsc-brand-select">
                  <div class="selection">
                    ...
                    <?//php if(!is_tpl_follow('brand_name')):?>
                      <em class="add-on" nctype="add-on"><i class="icon-collapse"></i></em>
                    <?//php endif?>
            
                  </div>
                  <span><?//php if(is_tpl_follow('brand_name')) echo lang('goods_attr_admin_control');?></span>
                  <?//php if(!is_tpl_follow('brand_name')):?>
                ...
                <ul>
                  <?php //if(!empty($output['attr_list']) && is_array($output['attr_list'])){?>
                    ...
                    <li class="J_spu-property" id="name_<?//php echo $val['name_id'];?>">
                      ...
                      <span>
                        <?php 
                        //$attr_checked = !empty($output['attr_checked'])?$output['attr_checked']:'';
                        //if(is_tpl_follow('spu'))
                         // echo doView($val['input_type'],$val['name_id'], $val['valList'],$attr_checked );
                        //else
                        //  echo doInput($val['input_type'],$val['name_id'], $val['is_required'], $val['valList'],$attr_checked );?>
                      </span>
                    </li>
                    <?php// } ?>
                  <?php //}?>
          ...
          <?php //if(!is_tpl_follow('m_content')):?>
            <!-- <dl>
              <dt><?//php echo lang('store_goods_index_goods_desc').lang('nc_colon');?><br />
                <i class="hint"><?//php if(is_tpl_follow('content')) echo lang('goods_attr_admin_control');?></i>
              </dt>
              <dd id="ncProductDetails">
                <div class="tabs">
                  ...
                  <div id="panel-1" class="ui-tabs-panel" jquery1239647486215="4">
                    <?php
                    $content// = empty($output['goods']['content'])?'':$output['goods']['content'];
                    //showEditor('content', $content,'100%','480px','visibility:hidden;',"false",empty($output['editor_multimedia'])?'':$output['editor_multimedia']);?>
                  <?php //if(!is_tpl_follow('content')):?>
                    <div class="hr8">
                      <div class="ncsc-upload-btn"> <a href="javascript:void(0);"><span>
                      ...
                      <a class="ncsc-btn mt5" nctype="show_desc" href="<?php// echo urlShop('seller/goods_album', 'pic_list', array('item'=>'des'));?>"><i class="icon-picture"></i><?php echo lang('store_goods_album_insert_users_photo');?></a> <a href="javascript:void(0);" nctype="del_desc" class="ncsc-btn mt5" style="display: none;"><i class=" icon-circle-arrow-up"></i>关闭相册</a>
                    </div>
                  <?php// endif?>
                    <p id="des_demo"></p>
                  </div>
                    ...
                    <div class="ncsc-mobile-editor">
                      <div class="pannel">
                        ...
                        <div class="control-panel" nctype="mobile_pannel">
                          <?php 
                          $m_content// = empty($output['goods']['m_content'])?'':$output['goods']['m_content'];
                          //if (!empty($m_content)) {?>
                          <?php $m_content// = json_decode($m_content,true);
          ...
          <?php //endif?>
            ...
            <!-- 商品物流信息 E -->
            <input name="g_state" value="1" type="hidden">
            <!--
            ...
            <dl>
              ...
              <dd>
                <ul class="ncsc-form-radio-list">
                  <li>
                    <label>
                      <input name="g_state" value="1" type="radio" <?php if (empty($output['goods']['status']) || $output['goods']['status'] == 1) {?>checked="checked"<?php }?> />
                      <?php echo lang('store_goods_index_immediately_sales');?> </label>
                  </li>
                  <li>
                    <label>
                      <input name="g_state" value="4" type="radio" nctype="auto" <?php if (!empty($output['goods']['status']) && $output['goods']['status'] == 4) {?>checked="checked"<?php }?>/>
                  ...
                  <li>
                    <label>
                      <input name="g_state" value="3" type="radio" <?php if (!empty($output['goods']['status']) && $output['goods']['status'] == 3) {?>checked="checked"<?php }?> />
...
//console.log(img);
      ...
      function recursionSpec($len,$sign) {
          if($len < $sign){
              ...
              echo "spec_bunch = spec_bunch.substring(0,spec_bunch.length-1);";
              ...
              echo "str +='<td><input class=\"text price\" type=\"text\" name=\"spec['+spec_bunch+'][price]\" data_type=\"price\" nc_type=\"'+spec_bunch+'|price\" value=\"\" /><em class=\"add-on\"><i class=\"icon-renminbi\"></i></em></td><td><input class=\"text price\" type=\"text\" name=\"spec['+spec_bunch+'][costprice]\" nc_type=\"'+spec_bunch+'|costprice\" value=\"\" /><em class=\"add-on\"><i class=\"icon-renminbi\"></i></em></td><td><input class=\"text stock\" type=\"text\" name=\"spec['+spec_bunch+'][stock]\" data_type=\"stock\" nc_type=\"'+spec_bunch+'|stock\" value=\"\" /></td></tr>';\n";
              for($i=0; $i< $len; $i++){
                  echo "}\n";
              }
          }
      ...
      <?php if (!empty($output['goods']) && empty($_GET['tpl_id']) && !empty($output['spec_checked']) && !empty($output['spec_list']) ){?>
      //  编辑商品时处理JS
      $(function(){
        var E_SP = new Array();
